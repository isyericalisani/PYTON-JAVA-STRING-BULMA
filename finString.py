...
def find_strings_in_java_files(directory):
    data = {"File Name": [], "String Expression": [], "Full Line": []}
     
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith(".java"):
                file_path = os.path.join(root, file_name)
                extract_strings_from_file(file_path, data, file_name)
...
def is_valid_string(string):
    # Filtreleri uygula
    invalid_keywords = ["(",")","%","=","ZIP","XML","YYYY","ENCODE","HTTP","XMLN","PNG","+","STRING","GIF","THEN","ADMIN","TSL","FORMAT",">","<","WHEN","IS NULL", "OR ", "T_", "V_", "LEFT", "CASE", "TO_STRING", "GUNLE", "CURRENCY", "FIELD", "LABEL", "FORMATTED", "TO_CHAR", "AND", "##", "BOX", "PIE", "SELECT", "ORDER BY", "JASPER", "PANE", "BLS", "GNL", "TR.", "FROM", "WHERE", "A."]
...
def extract_strings_from_file(file_path, data, file_name):
            ...
            if strings:
                for string in strings:
                    if is_valid_string(string) and is_valid_line(line):
                        ...
                        data["Full Line"].append(line.strip())
